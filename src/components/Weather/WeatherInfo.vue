<template>
  <div id="weather-info" v-if="this.dataOfWeather != ''">
    ...
    <span style="font-family: 'Rubik Medium', sans-serif; font-size: 96px; font-weight:500">
      {{ this.dataOfWeather.current.temp_c }}°C
    </span>
    <span>{{ this.dataOfWeather.current.condition.text }}</span>
  </div>
  <div id="other-info" v-if="this.dataOfWeather != ''">
    <WeatherCard
      title='Ветер'
      unit="м/с"
      :value="this.dataOfWeather.current.wind_mph">
    ...
    <WeatherCard
      title='Влажность'
      unit="%"
      :value="this.dataOfWeather.current.humidity">
    ...
    <WeatherCard
      title='Осадки'
      unit="мм"
      :value="this.dataOfWeather.current.precip_mm">
      <svg width="11" height="11" viewBox="0 0 11 11" fill="none" xmlns="http://www.w3.org/2000/svg">
